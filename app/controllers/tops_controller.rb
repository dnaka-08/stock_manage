class TopsController < ApplicationController
  ...
  def index
    if @user.admin
      ...
      @stocks = Stock.joins(:store, :product).where('stocks.store_id = ? and date <= ?', @user.store_id, Time.zone.now).group("stocks.store_id").group("stocks.product_id").group("stores.name").group("products.name").select("stocks.store_id, stocks.product_id, sum(stocks.total_number) as total_number, stores.name as store_name, products.name as product_name")
    end
